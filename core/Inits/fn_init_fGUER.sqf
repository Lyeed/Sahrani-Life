...
if ((isNil "g_GUERLevel") || ((call g_GUERLevel) isEqualTo 0)) exitWith 
{
	["Ce slot est réservé, vous devez être sur la whiteliste pour pouvoir vous y connecter"] spawn public_fnc_errorExit;
	false;
};

if ((((call g_EASTLevel) > 0) || ((call g_REBLevel) > 0) || ((call g_WESTLevel) > 0)) && ((call g_adminlevel) isEqualTo 0)) exitWith
{
