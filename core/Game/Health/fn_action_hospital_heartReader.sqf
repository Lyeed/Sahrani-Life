	...
	if (_reader getVariable ["monitor_on", false]) then
	{
		if (player getVariable ["heart_attack", false]) then
		{
			if ((_reader animationPhase "reader_PulseOff") != 1) then
			{
				...
				_reader animate ["reader_PulseOff", 1];
			};
			if ((_reader animationPhase "reader_PulseOn") != 0) then {_reader animate ["reader_PulseOn", 0]};
		} else {
			if ((_reader animationPhase "reader_PulseOff") != 0) then {_reader animate ["reader_PulseOff", 0]};
			if ((_reader animationPhase "reader_PulseOn") != 1) then {_reader animate ["reader_PulseOn", 1]};
