...
_data = ([g_AH_type, g_AH_location] call AlysiaClient_fnc_auctionHouse_getStock) select _index_global;
if (isNil "_data") then {
	["Impossible de trouver les informations de vente<br/>L'objet vient peut être d'être acheté ou a déjà été retiré de la vente"] call AlysiaClient_fnc_error;
} else {
	if ((lbData[45621, _index_stock]) isEqualTo (_data select 0)) then
	{
		...
		if (_handle isEqualTo "") then
		{
			...
			[g_AH_type, g_AH_location, _data, _index_global, player] remoteExec ["AlysiaServer_fnc_auctionHouse_remove", 2];
			sleep 1.5;
		} else {
...
[2] call AlysiaClient_fnc_auctionHouse_menu_filter;
