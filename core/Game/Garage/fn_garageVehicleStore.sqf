...
if (_storeInventory) then
{
	if (_pay) then
	{
		...
		if (_price > 0) then
		{
			if (g_atm > _price) then
			{
				...
				playSound "buy";
				g_garage_store = true;
				[_vehicle, _storePos, false] remoteExec ["TON_fnc_garageVehicleStore", 2];
			} else {
				[format["Vous n'avez pas assez d'argent sur votre compte pour payer la facture du garage, vous demandant <t color='#8cff9b'>%1</t>kn pour stocker vos objets", ([_price] call public_fnc_numberText)]] call public_fnc_error;
			};
		} else {
			g_garage_store = true;
			[_vehicle, _storePos, false] remoteExec ["TON_fnc_garageVehicleStore", 2];
	...
	g_garage_store = true;
	[_vehicle, _storePos, true] remoteExec ["TON_fnc_garageVehicleStore", 2];	
};
