...
{
	if ([format[getText(missionConfigFile >> "ALYSIA_DYN_MARKERS" >> "destroy" >> "message"), (markerText _marker)], "Validation", "Valider", "Annuler"] call BIS_fnc_guiMessage) then
	{
		if ((player distance (getMarkerPos _marker)) < 20) then
		{
			_message = getText(missionConfigFile >> "ALYSIA_DYN_MARKERS" >> "destroy" >> "global_message");
			if (_message != "") then {
				(format[_message, getText(missionConfigFile >> "ALYSIA_FACTIONS" >> str(playerSide) >> "name"), (markerText _marker)]) remoteExecCall ["systemChat", -2];
			};
