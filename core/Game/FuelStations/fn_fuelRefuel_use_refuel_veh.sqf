...
while {dialog} do
{
	...
	if ((_bill + ([_station, (player getVariable ["typeRefuel", ""])] call public_fnc_fuelStation_price_buy)) > g_cash) exitWith
	{
	...
	_bill = (_bill + ([_station, (player getVariable ["typeRefuel", ""])] call public_fnc_fuelStation_price_buy));
