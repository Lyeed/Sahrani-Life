...
if (_zone isEqualTo "") then
{
	...
	{
		_count = [_x select 0] call public_fnc_itemCount;
	...
	{
		["Vous n'avez pas de graine"] call public_fnc_error;
		false breakOut "main";
	};
	...
	if (_zone isEqualTo "") then
	{
		["Aucune zone de récolte n'a été spécifiée"] call public_fnc_error;
		false breakOut "main";
	};
	...
	if (!([false, _seed, 1] call public_fnc_handleInv)) then
	{
...
if ([false, "engrais", 1] call public_fnc_handleInv) then {_plantGrowingtime = round(_plantGrowingtime * 0.7)};
