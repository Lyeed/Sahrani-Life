...
if (isNull _object) exitWith {
	["Aucune plante à récolter à proximité"] call AlysiaClient_fnc_error;
};
...
if (_object getVariable ["ready", false]) then
{
...

	{
		...
		if (_amount isEqualTo 0) then
		{
			if (_forEachIndex isEqualTo 0) then {
				...
				_missing = _missing + format["- <t color='#FF8000'>%1</t><br/>", [(_x select 0)] call AlysiaClient_fnc_itemGetName];
			};
		} else {
	...
	["Cette pousse n'est pas encore prête à être récoltée"] call AlysiaClient_fnc_error;
};
