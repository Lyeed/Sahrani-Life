...
[
	...
	[
		...
		["refuel", "Faire le plein", "[g_interaction_target, (missionNamespace getVariable[""refuel_station_inUse"", objNull])] spawn public_fnc_fuelRefuel_use_refuel_veh;", '(((player getVariable ["typeRefuel", ""]) != "") && !(isNull (missionNamespace getVariable["refuel_station_inUse", objNull])) && ((vehicle player) isEqualTo player) && ((locked g_interaction_target) != 2))'],
