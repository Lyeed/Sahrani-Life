...
[
	...
	[
		...
		["money_transfer", "Sac", "", "((getNumber(missionConfigFile >> 'ALYSIA_VEHICLES' >> typeOf(g_interaction_target) >> 'money_transfer') isEqualTo 1) && ((vehicle player) isEqualTo player))"],
		["money_bomb", "Braquer", "", "((getNumber(missionConfigFile >> 'ALYSIA_VEHICLES' >> typeOf(g_interaction_target) >> 'money_transfer') isEqualTo 1) && ((vehicle player) isEqualTo player))"]
	],
