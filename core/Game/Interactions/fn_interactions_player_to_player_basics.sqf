...
[
	...
	[
		["identity", "Carte d'identité", "[g_interaction_target] spawn AlysiaClient_fnc_showIdentity; closeDialog 0;", "((getText(missionConfigFile >> 'ALYSIA_FACTIONS' >> str(playerSide) >> 'identity_item') isEqualTo '') || (getText(missionConfigFile >> 'ALYSIA_FACTIONS' >> str(playerSide) >> 'identity_item') in (magazines player)))"],
		...
		["escort", "Escorter", "[g_interaction_target] spawn AlysiaClient_fnc_escort; closeDialog 0;", "((g_interaction_target getVariable ['restrained', false]) && (isNull (player getVariable ['escorting', objNull])) && (isNull (g_interaction_target getVariable ['escorted', objNull])))"],
		["stopescort", "Arrêter l'escorte", "[g_interaction_target] spawn AlysiaClient_fnc_stopescort; closeDialog 0;", "(!(isNull (g_interaction_target getVariable ['escorted', objNull])) || !(isNull (g_interaction_target getVariable ['escorting', objNull])))"],
		...
		["ticket", "Paiement", "[] spawn AlysiaClient_fnc_interactionMenu_action_ticket;", "((getNumber(missionConfigFile >> 'ALYSIA_FACTIONS' >> str(playerSide) >> 'interaction_ticket') isEqualTo 1) && (isNull (g_interaction_target getVariable ['escorted', objNull])))"],
