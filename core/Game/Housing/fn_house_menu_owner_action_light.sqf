...
if (isNull (g_interaction_target getVariable ["lightSource", ObjNull])) then 
{
	...
	[g_interaction_target, true] remoteExecCall ["public_fnc_house_menu_owner_action_light_update", -2];
} else {
	["<t align='center'>Lumières<br/><t color='#FF8000'>OFF</t></t>"] call public_fnc_info;
	[g_interaction_target, false] remoteExecCall ["public_fnc_house_menu_owner_action_light_update", -2];
};