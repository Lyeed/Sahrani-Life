...
if (_action) then 
{
	...
	if (isNull _storage) then
	{
		...
		[_target, (getPlayerUID player)] remoteExec ["AlysiaServer_fnc_house_sell", 2];
		["<t align='center'>Vente<br/><t color='#3ADF00'>effectuée</t></t>", "buy"] call AlysiaClient_fnc_info;
	} else {
		["Vous devez ranger le coffre de votre maison avant de la vendre."] call AlysiaClient_fnc_error;
	};
};
