...
if (!(["Inspection", 4, true, "", false, _object] call public_fnc_showProgress)) exitWith {};

if ((count(_object getVariable["info", []]) > 0) && (count(_object getVariable["users", []]) > 0)) then
{
	private["_users", "_msgTxt"];
	_users = _object getVariable["users", []];
	_msgTxt = "";
	{
