...
_val = ctrlText 10008;
if (!([_val] call AlysiaClient_fnc_isNumber)) exitWith {
	["La somme d'argent à donner doit être un nombre"] call AlysiaClient_fnc_error;
};
...
if (_val > 999999) exitWith {
	["Vous ne pouvez pas donner plus de <t color='8cff9b'>999.999kn</t> d'un coup"] call AlysiaClient_fnc_error;
};
