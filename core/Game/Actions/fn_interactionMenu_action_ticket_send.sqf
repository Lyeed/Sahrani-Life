...
if (!([_price] call public_fnc_isNumber)) exitWith {
	["La somme à payer doit être un nombre"] call public_fnc_error;
};
...
if (([_desc] call CBA_fnc_strLen) > 70) exitWith {
	["La description ne doit pas dépasser 70 caractères"] call public_fnc_error;
};

closeDialog 0;
["Paiement proposé"] call public_fnc_info;
[_price, _desc, player] remoteExec ["public_fnc_interactionMenu_action_ticket_receive", g_interaction_target];