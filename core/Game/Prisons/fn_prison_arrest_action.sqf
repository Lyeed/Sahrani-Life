...
if (_bail < _bailMin) exitWith {
	[format["La caution doit être supérieur à <t color='#8cff9b'>%1</t>kn.", _bailMin]] call AlysiaClient_fnc_error;
};
if (_bail > _bailMax) exitWith {
	[format["La caution doit être inférieur ou égal à <t color='#8cff9b'>%1</t>kn.", _bailMax]] call AlysiaClient_fnc_error;
};
