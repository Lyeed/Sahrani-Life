...
if (!(g_interaction_target getVariable ["construction", false])) exitWith {
	["Cette entreprise n'est pas en construction"] call public_fnc_error;
};

if (!((g_interaction_target getVariable ["construction_require", []]) isEqualTo [])) exitWith {
	["Cette entreprise a encore besoin de matériaux"] call public_fnc_error;
};

if ((g_interaction_target getVariable ["trunk_in_use_ID", ""]) != "") exitWith {
	["L'inventaire est en cours d'utilisation ou quelqu'un est déjà en train de construire l'entreprise"] call public_fnc_error;
};

g_interaction_target setVariable ["trunk_in_use_ID", "CONSTRUCTION", true];

if (!(["Construction", 15, g_interaction_target, "", "AinvPknlMstpsnonWnonDnon_medic_1"] call public_fnc_showProgress)) exitWith {
...
[g_interaction_target] remoteExecCall ["TON_fnc_company_build", 2];
