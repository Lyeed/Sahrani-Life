...
if ([false, _company, _uid] call AlysiaClient_fnc_company_member_handle) then
{
	...
	if (!(isNull _member)) then {
		[player] remoteExecCall ["AlysiaClient_fnc_company_member_leave", _member];
	};
} else {
	[format["Impossible de renvoyer la personne de l'entreprise. Il n'en fait peut être déjà plus parti."]] call AlysiaClient_fnc_info;
};
