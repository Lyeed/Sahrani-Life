...
{
	_index = _list_construction lbAdd format["%1x %2", ([(_x select 1)] call public_fnc_numberText), ([(_x select 0)] call public_fnc_itemGetName)];
	_list_construction lbSetPicture [_index, ([(_x select 0)] call public_fnc_itemGetImage)];
...
{
	if (([(_x select 0), g_interaction_require] call public_fnc_index) != -1) then
	{
		_index = _list_player lbAdd format["%1x %2", ([(_x select 1)] call public_fnc_numberText), ([(_x select 0)] call public_fnc_itemGetName)];
		_list_player lbSetData [_index, (_x select 0)];
		_list_player lbSetPicture [_index, ([(_x select 0)] call public_fnc_itemGetImage)];
		_list_player lbSetTooltip [_index, (_list_player lbText _index)];
	};
} forEach ([] call public_fnc_getInv);
