...
if ([_value] call public_fnc_isNumber) then
{
	...
	if (_value < 999999) then
	{
		if (_value < 0) then
		{
			...
			["Vous ne pouvez pas déposer une somme d'argent négative ou égale à zéro"] call public_fnc_error;
		};
	} else {
		[format["Vous ne pouvez pas déposer plus de <t color='#8cff9b'>%1</t> en une fois", ([999999] call public_fnc_numberText)]] call public_fnc_error;
	};
} else {
