...
if (!([_amount] call public_fnc_isNumber)) exitWith {
	["Erreur dans le montant"] call public_fnc_error;
};
...
switch (_action) do
{
	case "withdraw":
	{
		if (_amount > getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "withdraw_max")) exitWith {
			[format["Vous ne pouvez pas retirer plus de <t color='#8cff9b'>%1</t>kn.", [getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "withdraw_max")] call public_fnc_numberText]] call public_fnc_error;
		};
		if (_amount < getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "withdraw_min")) exitWith {
			[format["Vous ne pouvez pas retirer moins de <t color='#8cff9b'>%1</t>kn.", [getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "withdraw_min")] call public_fnc_numberText]] call public_fnc_error;
		};
		...
		if (g_atm < _tax) exitWith {
			[format["Vous n'avez pas assez d'argent dans votre compte pour payer la taxe de prélèvement de ce DAB s'élevant à <t color='#8cff9b'>%1</t>kn", [_tax] call public_fnc_numberText]] call public_fnc_error;
		};
		...
		if (_amount > _atm_money) exitWith {
			[format["Il n'y à pas assez d'argent dans le DAB pour retirer <t color='8cff9b'>%1</t>kn.<br/>Max : <t color='8cff9b'>%2</t>kn", [_amount] call public_fnc_numberText, [_atm_money] call public_fnc_numberText]] call public_fnc_error;
		};

		g_interaction_target setVariable ["money", (_atm_money - _amount), true];
		[false, _amount, "Retrait DAB"] call public_fnc_handleATM;
		[false, _tax, "Taxe prélèvenement DAB"] call public_fnc_handleATM;
	...
	case "deposit":
	{
		if (_amount > getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "deposit_max")) exitWith {
			[format["Vous ne pouvez pas déposer plus de <t color='#8cff9b'>%1</t>kn.", [getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "deposit_max")] call public_fnc_numberText]] call public_fnc_error;
		};
		if (_amount < getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "deposit_min")) exitWith {
			[format["Vous ne pouvez pas déposer moins de <t color='#8cff9b'>%1</t>kn.", [getNumber(missionConfigFile >> "ALYSIA_ATM" >> typeOf(g_interaction_target) >> "deposit_min")] call public_fnc_numberText]] call public_fnc_error;
		};
		if (g_cash < _amount) exitWith {
			["Vous n'avez pas assez de fonds sur vous"] call public_fnc_error;
		};
