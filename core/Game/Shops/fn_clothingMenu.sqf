...
*/
private["_list", "_filter", "_type"];
_type = [_this, 3, "", [""]] call BIS_fnc_param;

if (_type == "") exitWith {};
if (!isClass(missionConfigFile >> "ALYSIA_SHOPS_CLOTHING" >> _type)) exitWith {};

if ((call compile format["%1", (getText(missionConfigFile >> "ALYSIA_SHOPS_CLOTHING" >> _type >> "side"))]) != playerSide) exitWith { ["Vous n'avez pas accès à ce magasin"] call ALYSIA_fnc_error; };

if (!(createDialog "Life_Clothing")) exitWith {};
disableSerialization;

life_clothing_store = _type;
ctrlSetText [3103, getText(missionConfigFile >> "ALYSIA_SHOPS_CLOTHING" >> _type >> "name")];
