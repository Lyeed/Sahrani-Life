...
_bad = [_title, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz123456789-éè?!,.ù_à "] call AlysiaClient_fnc_TextAllowed;
...
_bad = [_pseudo, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz123456789-éè?!,.ù_à "] call AlysiaClient_fnc_TextAllowed;
...
_bad = [_message, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz123456789-éè?!,.ù_à "] call AlysiaClient_fnc_TextAllowed;
...
if (([_message] call CBA_fnc_strLen) > 80) exitWith {["Le message de votre annonce ne doit pas dépasser 80 caractères"] call AlysiaClient_fnc_error};
