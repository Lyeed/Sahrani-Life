...
[
	...
	[
		...
		([(side _from)] call public_fnc_sideToStr)
	],
	"Recrutement",
	"Accepter",
	"Refuser"
...
if (_action) then
{
	...
	[format["%1 a <t color='#01DF01'>accepté</t> votre invitation", player getVariable ["realname", profileName]]] remoteExecCall ["public_fnc_info", _from];
	...
	(format["%1 a <t color='#01DF01'>refusé</t> votre invitation", player getVariable ["realname", profileName]]) remoteExecCall ["public_fnc_info", _from];
};